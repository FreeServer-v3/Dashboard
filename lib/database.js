...
module.exports = {
	...
	getCycles: async function() {
		return new Promise(async (resolve, reject) => {
			const collection = db.collection('cycle');
	...
	getUserCycle: async function(user) {
		return new Promise(async (resolve, reject) => {
			const collection = db.collection('cycle')
			const userCycle = await collection.findOne({
				user: user,
			})
			if (!userCycle) {
				resolve(false)
	...
	setNextCycleExpire: async function(user, isInit) {
		return new Promise(async (resolve, reject) => {
			...
			if (!userCycle) {
				...
				let days = 0
				const expirytime = userCycle.next_expire
				...
				if (isInit) {
					...
					const olddate = new Date(expirytime);
					const newExpiryDate = new Date(olddate);
					newExpiryDate.setDate(newExpiryDate.getDate() + days)
					newTimeStamp = newExpiryDate.getTime();
				}
				await collection.updateOne({ user: user }, { $set: { 'next_expire': newTimeStamp } });
			}
			resolve(true)
		})
	},
	updateUserCycle: async function(user, cycle) {
		return new Promise(async (resolve, reject) => {
			const collection = db.collection('cycle');
			const checkUserExisits = await collection.findOne({
				user: user
			})
			if (!checkUserExisits) {
				reject(false)
				...
				await collection.updateOne({ user: user }, { $set: { 'cycle': cycle } });
			}
			resolve(true)
		})
	},
	createUserCycle: async function(user) {
		return new Promise(async (resolve, reject) => {
			const collection = db.collection('cycle');
			const checkExisits = await collection.findOne({
				user: user
			})
			if (checkExisits) {
				reject(false)
	...
	getSpecsPaid: async function(user) {
		return new Promise(async (resolve, reject) => {
			...
			const cyclecollection = db.collection('cycle');
			const settingscollection = db.collection('settings');
			
			const getCycle = await cyclecollection.findOne({
				user: user
			})
			if (!getCycle) {
				reject(false)
			}
			const getUserSpecs = await usercollection.findOne({
				email: user
			})
			const usrCPU = parseInt(getUserSpecs.extra.cpu);
			const usrRam = parseInt(getUserSpecs.extra.ram);
			const usrDisk = parseInt(getUserSpecs.extra.disk);
			
			const getSettings = await settingscollection.findOne({
				name: "Dashactyl",
			})
			...
			if (getCycle.cycle === daily) {
				cycleMultiplier = parseInt(getSettings.day_mutliplier);
			} else if (getCycle.cycle === weekly) {
				cycleMultiplier = parseInt(getSettings.week_multiplier);
			} else if (getCycle.cycle === monthly) {
				cycleMultiplier = 1;
			}
	...
	createCoupon: async function (couponCode, coins) {
		return new Promise(async (resolve, reject) => {
			const collection = db.collection('coupons');
			const checkCoupon = await collection.findOne({
				code: couponCode
			})
			if (checkCoupon) {
				resolve(false);
	...
	deleteCoupon: async function (couponCode) {
		return new Promise(async (resolve, reject) => {
			...
			if (!coupon) {
				...
				await collection.deleteOne({code: couponCode});
				resolve(true)
			}
		})
	},
